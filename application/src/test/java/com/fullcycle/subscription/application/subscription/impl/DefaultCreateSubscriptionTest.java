...
class DefaultCreateSubscriptionTest extends UnitTest {
    ...
    public void givenValidAccountAndPlan_whenCallsCreateSubscription_shouldReturnNewSubscription() {
        ...
        // when
        final var actualOutput = this.target.execute(new CreateSubscriptionTestInput(expectedAccount.id().value(), expectedPlan.id().value()));
        ...
        Assertions.assertTrue(actualSubscription.isTrial());
