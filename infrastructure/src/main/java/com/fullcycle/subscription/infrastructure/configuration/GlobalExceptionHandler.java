...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<?> handleInternalErrorException(InternalErrorException ex) {
        return ResponseEntity.unprocessableEntity().body(new Error("", ex.getMessage()));
    }
    ...
    private List<Error> convertError(List<ObjectError> allErrors) {
        return allErrors.stream().map(e -> new Error(((FieldError) e).getField(), e.getDefaultMessage())).toList();
    }
}
